...
const MAIN_FEED_IDS: PackedStringArray = ["front", "rear", "left", "right", "driver", "quad"]
const MINIMAP_ZOOM_STEP: float = 2.0

var _current_main_feed: String = "driver"
var _last_single_feed: String = "driver"
...
@onready var _main_panel_texture: Control = $RootLayout/MainArea/MainFeedPanel/VBoxContainer/TextureRect
@onready var _quad_grid: Control = $RootLayout/MainArea/QuadGrid
...
@onready var _quad_left_panel: Node = $RootLayout/MainArea/QuadGrid/Grid/QuadLeft
@onready var _quad_right_panel: Node = $RootLayout/MainArea/QuadGrid/Grid/QuadRight
...
func _ready() -> void:
	...
	_setup_quad_panel(_quad_front_panel, "front", "Front")
	_setup_quad_panel(_quad_rear_panel, "rear", "Rear")
	_setup_quad_panel(_quad_left_panel, "left", "Left")
	...
	if _minimap_panel.has_method("set_feed_name"):
		_minimap_panel.set_feed_name("")
...
func _input(event: InputEvent) -> void:
	...
	if mouse_button and mouse_button.pressed and mouse_button.button_index == MOUSE_BUTTON_LEFT and _current_main_feed == "driver":
		if _is_driver_toggle_click(mouse_button.global_position):
...
func _setup_quad_panel(panel: Node, feed_id: String, label_text: String) -> void:
	_set_panel_label(panel, label_text)
	if panel.has_method("set_feed_name"):
		panel.set_feed_name("")
	if panel.has_method("set_fov_controls_visible"):
		panel.set_fov_controls_visible(false)
	if panel.has_method("set_fov_locked"):
		panel.set_fov_locked(true)
...
func _refresh_camera_textures() -> void:
	...
	if _current_main_feed == "quad":
		_set_panel_texture(_quad_front_panel, _camera_rig.get_feed("front"))
		_set_panel_texture(_quad_rear_panel, _camera_rig.get_feed("rear"))
		_set_panel_texture(_quad_left_panel, _camera_rig.get_feed("left"))
		_set_panel_texture(_quad_right_panel, _camera_rig.get_feed("right"))
	else:
...
func _switch_main_feed(feed_id: String) -> void:
	...
	if feed_id != "quad" and _camera_rig and _camera_rig.has_method("get_feed"):
		_set_panel_texture(_main_panel, _camera_rig.get_feed(_current_main_feed))
	_apply_display_mode()
	_update_selection_highlight()
	_update_status_text()
...
func _toggle_quad_view() -> void:
	if _current_main_feed == "quad":
		...
		_switch_main_feed("quad")

func _apply_display_mode() -> void:
	var is_quad: bool = _current_main_feed == "quad"
	if _main_panel is Control:
		(_main_panel as Control).visible = not is_quad
	_quad_grid.visible = is_quad
...
func _is_driver_toggle_click(global_pos: Vector2) -> bool:
	if _main_panel_texture == null:
		return false
	if not _main_panel_texture.get_global_rect().has_point(global_pos):
		return false
	var hovered: Control = get_viewport().gui_get_hovered_control()
	if hovered == null:
		return false
	if hovered == _main_panel_texture:
		return true
	return _main_panel_texture.is_ancestor_of(hovered)
...
func _update_selection_highlight() -> void:
	if _current_main_feed == "quad":
		if _front_panel.has_method("set_selected"):
			_front_panel.set_selected(false)
		if _rear_panel.has_method("set_selected"):
			_rear_panel.set_selected(false)
		if _left_panel.has_method("set_selected"):
			_left_panel.set_selected(false)
		if _right_panel.has_method("set_selected"):
			_right_panel.set_selected(false)
		return
	if _front_panel.has_method("set_selected"):
